...
module GPCCMPTestKit
  module GPCCMPV010_CI_BUILD
    class GpccmpBodyHeightGroup < Inferno::TestGroup
      title 'Observation GP CCMP Body Height Tests'
      short_description 'Verify support for the server capabilities required by the GP CCMP Body Height.'
      description %(
  # Background

The GPCCMP Observation GP CCMP Body Height sequence verifies that the system under test is
able to provide correct responses for Observation queries. These queries
must contain resources conforming to the GP CCMP Body Height as
...
the [GP CCMP Body Height](https://gpccmp.csiro.au/ig/StructureDefinition/GPCCMPBodyHeight). Each element is checked against
...
read succeeds.
      ...
      id :gpccmp_v010_ci_build_gpccmp_body_height
      run_as_group

      def self.metadata
        @metadata ||= InfernoSuiteGenerator::Generator::GroupMetadata.new(YAML.load_file(
                                                                            File.join(__dir__, 'gpccmp_body_height',
