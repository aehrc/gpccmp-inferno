...
require_relative 'questionnaire_response/questionnaire_response_update_update_test'
require_relative 'questionnaire_response/questionnaire_response_new_update_update_test'

module GPCCMPTestKit
  module GPCCMPV010_CI_BUILD
    class QuestionnaireResponseGroup < Inferno::TestGroup
      title 'GP CCMP QuestionnaireResponse Tests'
      short_description 'Verify support for the server capabilities required by the GP CCMP QuestionnaireResponse.'
      description %(
  # Background

The GPCCMP GP CCMP QuestionnaireResponse sequence verifies that the system under test is
able to provide correct responses for QuestionnaireResponse queries. These queries
must contain resources conforming to the GP CCMP QuestionnaireResponse as
...
the [GP CCMP QuestionnaireResponse](https://gpccmp.csiro.au/ig/StructureDefinition/GPCCMPQuestionnaireResponse). Each element is checked against
...
read succeeds.
      ...
      id :gpccmp_v010_ci_build_questionnaire_response
      run_as_group

      def self.metadata
        @metadata ||= InfernoSuiteGenerator::Generator::GroupMetadata.new(YAML.load_file(
                                                                            File.join(__dir__,
                                                                                      'questionnaire_response', 'metadata.yml'), aliases: true
                                                                          ))
      end

      test from: :gpccmp_v010_ci_build_questionnaire_response_patient_search_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_questionnaire_search_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_patient_questionnaire_search_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_read_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_validation_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_must_support_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_reference_resolution_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_create_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_update_update_test
      test from: :gpccmp_v010_ci_build_questionnaire_response_update_new_update_test
    end
  end
end
